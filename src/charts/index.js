...
window.addEventListener("DOMContentLoaded", () => {
      ...
      series: [
        {
          ...
          tooltip: {
            trigger: "item",
            formatter: (params) => {
              return `${params.name}</br><div style='display:flex;justify-content: space-between;'><div>${params.percent}%</div></div>`;
            },
          },
          label: {
            show: false,
  ...
  async function animateValue(data) {
    if (data && data.base) {
      const { buildingTotal, chargePoleTotal, enterpriseTotal, parkingTotal } =
        data.base;
