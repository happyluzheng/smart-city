...
import { FlyLineShader } from "@/model/FlyLineShader.js";
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  loadManager(["gltf/city.glb", "gltf/ship.glb"], (modelList) => {
    modelList.forEach(async (obj) => {
      if (obj.url === "gltf/city.glb") {
        ...
        EventBus.getInstance().on("pieClick", (buildName) => {
          ...
          if (city.lastClick && city.lastOriginMat.length > 0) {
            console.log(city.lastOriginMat, "city.lastOriginMat");
          ...
          targetBuild.traverse((model) => {
            if (model.type === "Mesh") {
              ...
              model.material = new THREE.MeshBasicMaterial({
                color: new THREE.Color("#ea403f"),
              });
              // modifySelectCityMaterial(model); // 再给选中的小物体边线再设置上去
            }
          });
          // console.log(city.lastOriginMat, "material");
